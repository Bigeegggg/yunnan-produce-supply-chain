...
export async function POST(request: Request) {
  ...
  const { name, phone, notes, cooperation_record } = body;
  if (!name || !phone) return NextResponse.json({ error: "名称和电话为必填项" }, { status: 400 });
  const db = getDb();
  const result = db.prepare("INSERT INTO supplier_contacts (name, phone, notes, cooperation_record) VALUES (?, ?, ?, ?)").run(name, phone, notes || "", cooperation_record || "");

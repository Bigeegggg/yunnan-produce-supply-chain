...
function getVideoEmbed(url: string, platform: string): string {
  if (platform === "youtube") { const match = url.match(/(?:v=|\/)([a-zA-Z0-9_-]{11})/); if (match) return `https://www.youtube.com/embed/${match[1]}`; }
  if (platform === "bilibili") { const match = url.match(/BV[a-zA-Z0-9]+/); if (match) return `https://player.bilibili.com/player.html?bvid=${match[0]}&page=1&high_quality=1`; }
  return url;
}

// This is a server component so we can't use useT. Use imported strings directly for metadata.
// For server components, we'll load translations from settings.
